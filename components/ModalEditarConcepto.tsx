...
interface ModalEditarConceptoProps {
    isOpen: boolean;
    onClose: () => void;
    concepto: Concepto | null;
    onSave: (id: number, data: Omit<Concepto, 'id'>) => void;
}

const ModalEditarConcepto: React.FC<ModalEditarConceptoProps> = ({ isOpen, onClose, concepto, onSave }) => {
    ...
    useEffect(() => {
        if (concepto) {
            setFormData({
                area: concepto.area || 'agua',
                clave: concepto.clave || '',
                nombre: concepto.nombre || '',
                precio: concepto.precio || 0,
                calculado: concepto.calculado || false,
                frecuencia_cobro: concepto.frecuencia_cobro || 'unico'
            });
        }
    }, [concepto, isOpen]);

    if (!isOpen || !concepto) return null;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await onSave(concepto.id, formData);
            onClose();
        } catch (err) {
            console.error(err);
        } finally {
            ...
            <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-white/20">
                <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="font-bold text-slate-800 text-lg">Editar Concepto de Cobro</h3>
                            <p className="text-xs text-emerald-800 font-bold uppercase tracking-wider">Modificando ID: {concepto.id}</p>
                        </div>
                    </div>
